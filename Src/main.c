...
void task_1(void)
{
    while (1)
    {
        if (USB_STATUS_CONFIGURED == usbfs_core_dev.dev.status)
        {
            memcpy(usb_data_buffer, "test\n", 5);
            cdc_acm_data_send(&usbfs_core_dev, 5);
            vTaskDelay(1000);
        }
    }
}
...
int main(void)
{
    xTaskCreate((void *)task_0, "task_0", configMINIMAL_STACK_SIZE, 0, 0, 0);
    xTaskCreate((void *)task_1, "task_1", configMINIMAL_STACK_SIZE, 0, 0, 0);
    vTaskStartScheduler();
}
